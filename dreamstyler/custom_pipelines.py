...
class EdgeDetectionFilter(nn.Module):
    def __init__(self):
        super(EdgeDetectionFilter, self).__init__()
        self.vertical = torch.tensor([[-1, 0, 1],
                                [-2, 0, 2],
                                [-1, 0, 1]], dtype=torch.float32).unsqueeze(0).unsqueeze(0)

        self.horizontal = torch.tensor([[-1, -2, -1],
                                  [0, 0, 0],
                                  [1, 2, 1]], dtype=torch.float32).unsqueeze(0).unsqueeze(0)

    def forward(self, x):
        horizontal_edges = F.conv2d(x, self.horizontal.expand(3, -1, -1, -1), padding=1, groups=3)
        vertical_edges = F.conv2d(x, self.vertical.expand(3, -1, -1, -1), padding=1, groups=3)

        edges = torch.sqrt(horizontal_edges**2 + vertical_edges**2)
...
def func_call_controlnet_img2img_pipeline(
    ...
    transform_edge = EdgeDetectionFilter()
    
    gray_image = transform_grayscale(image).repeat(1, 3, 1, 1)
    edges = transform_edge(gray_image)

    control_image = torch.where(edges > threshold, edges, torch.zeros_like(edges)).squeeze()
